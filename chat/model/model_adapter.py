    ...
    def raise_warning_for_old_weights(self, model):
        if isinstance(model, LlamaForCausalLM) and model.model.vocab_size > 32000:
            warnings.warn(
                ...
                "2. Use the old conversation template by `python3 -m chat.server.cli --model-path /path/to/vicuna-v0 --conv-template one_shot`\n"
...
class T5Adapter(BaseModelAdapter):
    """The model adapter for lmsys/chat-t5-3b-v1.0"""
...
class CodeLlamaTestGPTAdapter(BaseModelAdapter):
    """The model adapter for Code Llama"""

    def match(self, model_path: str):
        return "testgpt" in model_path.lower()
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        print("testgpt!!!!!!")
        return get_conv_template("llama-2-testgpt")
...
register_model_adapter(CodeLlamaTestGPTAdapter)
